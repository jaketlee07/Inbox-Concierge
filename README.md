...
| DB                 | **Supabase Postgres + RLS**                                | Per-user data isolation is enforced _in the database_, not the application — even a buggy route can't leak another user's threads.                                                                                             | RLS policies are easy to write wrong; oauth_tokens deliberately has _no_ policies and is service-role-only.                              |
