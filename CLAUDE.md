...
- Next.js 16: the `middleware.ts` file convention was renamed to `proxy.ts` and the exported function is `proxy(request)` (not `middleware(request)`). The old name still works but emits a deprecation warning. Use `proxy.ts`.
